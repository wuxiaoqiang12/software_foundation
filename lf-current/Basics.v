...
Notation "x <? y" := (ltb 2 2)(at level 70) : nat_scope.
...
(* 请在此处解答 *)
Example test_incr1: incr (B(B Z)) = A (A (B Z)).
Proof. simpl. reflexivity. Qed.
Example test_incr2: incr (A (A (B Z))) = B (A (B Z)).
Proof. simpl. reflexivity. Qed.
Example test_bin_to_nat1 : bin_to_nat (B (A (B Z))) =4.
Proof. simpl. reflexivity. Qed.
(* 请勿修改下面这一行： *)
